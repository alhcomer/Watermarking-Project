from tkinter import END, Button, OptionMenu, StringVar, Text, Tk, Toplevel, filedialog
...
def error_window():
    ...
    Label(
        window,
        text="Watermark Generator was unable to open that file. Ensure the file type is an image.",
    ).grid(row=0, column=0)
...
class MainFrame(Frame):
    def __init__(self, master, image, text, font, colour):
        ...
        Button(self, text="Upload a File", command=self._open_file).grid(
            row=1, column=1
        )
    ...
    def _open_file(self):
        self.file_path = filedialog.askopenfilename(
            initialdir="/",
            filetypes=(("JPEG", "*.jpg"), ("PNF", "*.png"), ("All files", "*.*")),
        )
...
class CheckImageFrame(Frame):
    def __init__(self, master, image, text, font, colour):
        ...
        Label(self, text="Is this the image you would like to watermark?").grid(
            column=0, row=1
        )
        self.button_frame = Frame(master=self)
        Button(self.button_frame, text="Yes", command=self._choose_watermark).grid(
            column=0, row=0, padx=10
        )
        Button(self.button_frame, text="No", command=self._to_first_frame).grid(
            column=1, row=0, padx=10
        )
        self.button_frame.grid(column=0, row=2, pady=10)

    def _choose_watermark(self):
        # TODO: add error window if text not validated

        if self.toplevel == None:
            self.toplevel = Toplevel()
            label = Label(
                self.toplevel,
                text="What text would you like to watermark the image with?",
            )
            ...
            self.font_select = OptionMenu(
                self.font_frame, self.variable, *list(self.font_options.keys())
            )
            ...
            self.colour_button = Button(
                self.colour_frame, text="Select a Colour", command=self._select_colour
            )
    ...
    def _select_colour(self):
        self.colour_pick = askcolor(
            color=None, title="Watermark Generator Colour Chooser"
        )
    ...
    def _to_check_wm(self):
        ...
        if (
            len(self.water_mark_text) < 4
            and self.water_mark_text
            and self.water_mark_text.strip()
        ):
...
class CheckWaterMarkFrame(Frame):
    def __init__(self, master, image, text, font, colour):
        ...
        Label(self, text="Is this image watermarked how you would like?").grid(
            column=0, row=1
        )
        self.button_frame = Frame(master=self)
        Button(self.button_frame, text="Yes", command=self._download_image).grid(
            column=0, row=0, padx=10
        )
        Button(self.button_frame, text="No", command=self._back_to_check_image).grid(
            column=1, row=0, padx=10
        )
    ...
    def watermark_image(self):
        ...
        for x in range(0, image_width, 150):
            for y in range(0, image_height, 150):
                self.watermark_image.paste(
                    ImageOps.colorize(w, (0, 0, 0), self.colour), (x, y), w
                )
